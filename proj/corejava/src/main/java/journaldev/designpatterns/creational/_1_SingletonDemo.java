...
I)	Eager Initialization
	creating singleton instance while declaring singleton class variable. Ex. java.lang.Runtime
	-) singleton instance will be created while loading class.
	-) No Exception handling

II)	Static Block Initialization
	creating singleton instance within static block with exception handling.
	-) singleton instance will be created while loading class.
	+) Exception handling
III)Lazy Initialization
	Creating singleton instance when we about to use first time.(in method)
...
IV)ThreadSafe Singleton
	Creating singleton instance when we about to use first time.(in synchronized method)
	+) Thread sfae
	-) synchronized method overhead cost is more.
	-) low performance due to synchronized

V) Bill Pugh Singleton
	it uses inner static helper class for creating singleton class instance. when the singleton class is loaded,
	inner helper class is not loaded into memory and only when someone calls the getInsatance()	method, this class gets loaded and creates the Singleton class instance. 
	+) This is the most widely used approach no need of synchronization
...
VI) Serialization Singleton
	if we want to store state of singleton object in file system in distributed environment. we have to implement Serializable interface.
	-) while deserializing object from file it creates new instance. so Singleton breaks.
	

VII) EnumSingletone
	+) Java ensures that any enum value is instantiated only once in a Java program. Since java Enums are globally accessible, so is the singleton.
